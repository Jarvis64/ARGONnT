...
def get_composite():
    """ Get Argon nucleus composite
    """
    from .Recoil_calculations import TARGET
    ArNuc = [TARGET(ATOM_TABLE["Ar40"], pure = True)]
    return ArNuc
...
class SN_lightcurve:
    ...
    def _get_rate1D(self, energies, t0=None, tf=None, dist=None):
        ...
        # convert MeV -> keV, and
        # multiply by the total number of Ar atoms in the TPC
        _nu1 = [ar.dRatedErecoil_vect(energies, Flux = flux_interpolator[nu_keys[0]])*(1e-3)*N_Ar.value for ar in ArNuc]
        _nu2 = [ar.dRatedErecoil_vect(energies, Flux = flux_interpolator[nu_keys[1]])*(1e-3)*N_Ar.value for ar in ArNuc]
        _nu3 = [ar.dRatedErecoil_vect(energies, Flux = flux_interpolator[nu_keys[2]])*(1e-3)*N_Ar.value for ar in ArNuc]
    ...
    def _get_rate2D(self, Rec_en, dist, step):
        ...
        for i, _Er in enumerate(tqdm(Ebins)):
            ...
            # convert MeV->keV + multiply by Nr of Ar atoms
            raw_rate1 = [ar.dRatedErecoil2D_vect(Er = _Er, Flux = interpolator[nu_keys[0]], t=tbins)*1e-3*N_Ar.value for ar in ArNuc]
            raw_rate2 = [ar.dRatedErecoil2D_vect(Er = _Er, Flux = interpolator[nu_keys[1]], t=tbins)*1e-3*N_Ar.value for ar in ArNuc]
            raw_rate3 = [ar.dRatedErecoil2D_vect(Er = _Er, Flux = interpolator[nu_keys[2]], t=tbins)*1e-3*N_Ar.value for ar in ArNuc]
            # for each isotope the rates are calculated wrt their fraction
            # Thus, multiplying each with N_Ar makes sense.
...
# model.N_Ar can be called to access N_Ar in constants.py
